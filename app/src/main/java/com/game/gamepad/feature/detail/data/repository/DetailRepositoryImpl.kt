...
class DetailRepositoryImpl(
    ...
    override fun getTrailer(gameId: String): Flow<Resource<List<Trailer?>?>> {
        return flow {
            emit(Resource.Loading(true))
            try {
                val items = api.getTrailer(gameId).toTrailers()
                emit(Resource.Success(items))
            } catch (e: IOException) {
                emit(
                    Resource.Error(
    ...
    override fun getScreenShots(gameId: String): Flow<Resource<List<ScreenShot?>?>> {
        return flow {
            emit(Resource.Loading(true))
            try {
                val items = api.getScreenShots(gameId).toScreenShotsList()
                emit(Resource.Success(items))
            } catch (e: IOException) {
                emit(
                    Resource.Error(
